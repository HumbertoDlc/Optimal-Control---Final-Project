...
q4=pi/8;    q4dot=0;    u4=0;
q5=pi/4;    q5dot=0;    u5=0;
syms z;
...
h=0;
g=5*(150-U(1))^2+5*(150-U(2))^2+U(6)^2+50*q1dot^2+50*q2dot^2;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%% Hamiltonian:
syms p1 p2 p3 p4 p5 p6 p7 p8 p9 p10
p3=0;p4=0;p5=0;p8=0;p9=0;p10=0;
H=g+p1*(x_dot(1))+p2*(x_dot(2))+p3*(x_dot(3))+p4*(x_dot(4))+p5*(x_dot(5))+...
    p6*(x_dot(6))+p7*(x_dot(7))+p8*(x_dot(8))+p9*(x_dot(9))+p10*(x_dot(10));
p1_dot=vpa(simplify(-diff(H,q1)),2);
p2_dot=vpa(simplify(-diff(H,q2)),2);
p3_dot=0;
p4_dot=0;
p5_dot=0;
p6_dot=vpa(simplify(-diff(H,q1dot)),2);
p7_dot=vpa(simplify(-diff(H,q2dot)),2);
p8_dot=0;
p9_dot=0;
p10_dot=0;
u_star_1=vpa(simplify(diff(H,u1)),2);
u_star_2=vpa(simplify(diff(H,u2)),2);
u_star_3=0;
u_star_4=0;
u_star_5=0;
